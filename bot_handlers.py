...
def task_date(message, task_text):
    ...
    try:
        ...
        button_week = types.InlineKeyboardButton(
            'Неделя',
            callback_data=f'{task_text} week'
        )
        button_month = types.InlineKeyboardButton(
            'Месяц',
            callback_data=f'{task_text} month'
        )
        no_repeatable = types.InlineKeyboardButton(
            'Без повторения',
            callback_data='only_one'
        )
        markup.add(button_week, button_month, no_repeatable)
        bot.send_message(message.chat.id, 'добавлено')
        bot.send_message(
            message.chat.id,
            'Выберите период повторения:',
            reply_markup=markup
        )
    except ValueError:
...
def get_user_week_tasks(message):
    ...
    db.close()
    if len(week_tasks) == 0:
        bot.send_message(message.chat.id, "У вас нет задач на эту неделю")
    else:
        bot.send_message(
            message.chat.id,
...
def task_delete(message):
    ...
    if len(result) == 0:
        bot.send_message(
            message.chat.id,
            'Такой задачи не найдено проверте правильность написания')
    ...
    for i in result:
        buttons.append(
            types.InlineKeyboardButton(
                f'{i[1]}', callback_data=f' *task_edit* {i[0]} * {i[1]}'
            )
        )
...
def get_day_tasks(message):
    ...
    db.close()
    if len(day_tasks) == 0:
        bot.send_message(message.chat.id, "У вас нет задач на сегодня")
    else:
        bot.send_message(
            message.chat.id,
            'Сегодня вам нужно сделать следующик задачи'
        )
    for el in day_tasks:
...
def get_month_tasks(message):
    db = BotDb('dairy_db.sql')
    month_tasks = db.get_month_tasks(message.chat.id)
    db.close()
    for el in day_tasks:
        bot.send_message(
            message.chat.id,
...
def get_today_tasks_statistic(message):
    ...
    for i in today:
        ...
        bot.send_message(
            message.chat.id,
            f"На задачу: {task} вы потратили {time} минут"
        )
...
def get_week_tasks_statistic(message):
    ...
    bot.send_message(
        message.chat.id,
        'Вот важи результаты на текушую неделю'
    )
    for i in week_stat:
        task = i[0]
        time = i[1]
...
def done_today_tasks(message):
    ...
    for i in day_tasks:
        data = '' + i[0] + '&' + i[1]
        buttons.append(
            types.InlineKeyboardButton(text=i[0][:12],
                                       callback_data=data))
