...
def cycle_month(call):
    ...
    buttons = [
        types.InlineKeyboardButton(
            day, callback_data=f'{current_task} * {day} * month_daily'
...
def get_start_point_period_statistic(message):
    bot.send_message(
        message.chat.id,
        'Введите дату начала отрезка статистики в формате "2023-01-01"'
    )
    ...
    bot.send_message(
        message.chat.id,
        "Вы можете вписать дату или выбрать кнопкой",
        reply_markup=markup)
    bot.register_next_step_handler(
        message, get_end_point_period_statistic
    )
...
def get_end_point_period_statistic(message):
    bot.send_message(
        message.chat.id,
        'Введите дату конца отрезка статистики в формате "2023-01-01"'
    )
    month = get_all_days_of_current_month()
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add(*month)
    ...
    bot.register_next_step_handler(
        message, get_period_tasks_statistic,
...
def get_period_tasks_statistic(message, start_point):
    ...
    period = db.get_task_statistic_period(
        message.chat.id, start_point, end_point
    )
    ...
    for i in period:
        ...
        time = i[1]
        bot.send_message(
            message.chat.id, f"На задачу: {task} вы потратили {time} минут"
        )

    bot.send_message(
        message.chat.id, f'''В период от {start_point} до {end_point}
...
def get_total_time_statistic_month(message):
    ...
    bot.send_message(
        message.chat.id, f'В этом месяце вы продуктивно провели'
                         f'{month_total_time} минут')
...
def get_total_time_statistic_week(message):
    ...
    bot.send_message(
        message.chat.id,
        f'На этой неделе вы продуктивно провели {month_total_time} минут')
...
def add_start_time(message, task_time_add, date_data):
    ...
    bot.send_message(
        message.chat.id,
        f'Ведите время в которое вы закончили выполнять задачу\n'
        f'{task_time_add} в формате "ЧЧ:ММ"')
    ...
    markup.add(*hours)
    bot.send_message(
        message.chat.id, "Вы можете вписать дату или выбрать кнопкой",
        reply_markup=markup
    )

    bot.register_next_step_handler(
        message, add_end_time,
        task_time_add=task_time_add,
...
def add_end_time(message, task_time_add, start_time, date_data):
    ...
    try:
        ...
        bot.send_message(
            message.chat.id,
            "Введите корректное время. Время должно быть больше 0."
        )
        sys.exit()
...
def delete_all_tasks_by_user_id(message):
    ...
    buttons = [
        types.InlineKeyboardButton(
            'Я хочу удалить все данные',
            callback_data=f'DELETE_ALL_YES*{message.chat.id}'),
        types.InlineKeyboardButton(
            'Я передумал удалять все данные',
            callback_data=f'DELETE_ALL_NO*{message.chat.id}')
    ]
    markup.add(*buttons)
...
def confirmation_delete_all_tasks_by_user_id(message):
    ...
    markup.add(*buttons)
    bot.edit_message_text(
        'АСТАНАВИТЕСЬ!', message.chat.id, message.message_id,
        reply_markup=markup)
...
@bot.callback_query_handler(
    func=lambda call: (
        call.data is not None and
        BotDb('dairy_db.sql').get_day_tasks(
            call.message.chat.id) is not None and
        call.data in BotDb('dairy_db.sql').get_day_tasks(
...
def done_today_callback(call):
    ...
    bot.send_message(
        call.message.chat.id,
        f''' Ведите время в которое вы начали выполнять задачу
        {task_time_add}\n
        в формате "ЧЧ:ММ"''')
    ...
    markup.add(*hours)
    bot.send_message(
        call.message.chat.id, "Вы можете вписать дату или выбрать кнопкой",
        reply_markup=markup
    )

    bot.register_next_step_handler(
        call.message, add_start_time,
        task_time_add=task_time_add,
        date_data=date_data)
...
def callback_recurring_tasks_week_handler(call):
    ...
    buttons = [
        types.InlineKeyboardButton(
            day, callback_data=f'{current_task} * {day}'
        ) for day in days
    ]
    next_week_button = types.InlineKeyboardButton(
        'Следующая неделя',
        callback_data=f'{current_task} *next_week_recurring'
    )
    buttons.append(next_week_button)
    markup.add(*buttons)

    text = '''Выберите дни недели в которые будет повторятся ваша задача
    от сегодняшнего дня и до конца недели.\n
...
def callback_recurring_tasks_next_week_handler(call):
    ...
    buttons = [
        types.InlineKeyboardButton(
            day, callback_data=f'{current_task} * {day} *next') for day in days
    ]
    markup.add(*buttons)

    text = '''Выберите дни недели в которые будет повторятся ваша задача
    от понедельника и до конца недели.\n
...
def all_callbacks_handler(call):
    ...
    try:
        if (len(call.data.split('*')) > 2 and
                call.data.split('*')[2].strip() == 'week'):
            task_text_range_week = (db.get_task_range_week(
                call.data.split('*')[1].strip(), call.message.chat.id))
    except TypeError:
        task_text_range_week = None

    try:
        if (len(call.data.split('*')) > 2 and
                call.data.split('*')[2].strip() == 'month'):
            task_text_range_month = (
                BotDb('dairy_db.sql').get_task_range_month(
                    call.data.split('*')[1].strip(),
                    call.message.chat.id))
    except TypeError:
        task_text_range_month = None
    ...
    try:
        if (len(call.data.split('*')) > 2 and
                call.data.split('*')[2].strip() == 'month_daily'):
            task_text_range_month = (
                BotDb('dairy_db.sql').get_task_range_month(
                    call.data.split('*')[0].strip(),
                    call.message.chat.id))
    except TypeError:
        task_text_range_month = None
    ...
    if (
            call.data is not None and
            today_tasks is not None and
            call.data.split("&")[0] in today_tasks):
    ...
    if len(call.data.split('*')) == 3:
        if (task_text_range_week is not None
            ...
            callback_recurring_tasks_week_handler(call)
    if len(call.data.split('*')) == 3:
        if (task_text_range_month is not None and
                call.data.split('*')[1].strip() == task_text_range_month and
                call.data.split('*')[
                    2].strip() == 'month'):
            cycle_month(call)

    if (3 > len(call.data.split('*')) > 1 and
    ...
    if (len(call.data.split('*')) > 1 and
            call.data.split('*')[1].strip() == 'next_week_recurring'):
        if (BotDb('dairy_db.sql').get_task_range_week(
                call.data.split('*')[0].strip(), call.message.chat.id)):
            callback_recurring_tasks_next_week_handler(call)
    if (len(call.data.split('*')) > 2 and
            call.data.split('*')[1].strip() in days and
            call.data.split('*')[2].strip() == 'next'):
        if (BotDb('dairy_db.sql').get_task_range_week(
                call.data.split('*')[0].strip(), call.message.chat.id)) and (
                call.data.split('*')[1].strip() in days):
            recurring_tasks_next_week(call)

    if call.data == 'only_one':
        bot.send_message(call.message.chat.id, 'Задача добавлена')
    if len(call.data.split('*')) == 3:
        if (task_text_range_month is not None and
                call.data.split('*')[0].strip() == task_text_range_month and
                call.data.split('*')[2].strip() == 'month_daily'):
            month_cycle_add(call)

    # вызывет функцию редактирования задачи
    if (len(call.data.split('*')) >= 3 and
            call.data.split('*')[1:][0] == 'task_edit'):
        if ((call.data.split('*')[1:][1].strip(),
             call.data.split('*')[1:][2].strip())
                in BotDb('dairy_db.sql').get_task_editing(
                    call.message, call.data.split('*')[1:][1].strip())):
    ...
    if (len(call.data.split('*')) >= 3 and
            call.data.split('*')[1:][0] == 'task_delete'):
        if ((call.data.split('*')[1:][1].strip(),
             call.data.split('*')[1:][2].strip())
                in BotDb('dairy_db.sql').get_task_editing(
                    call.message, call.data.split('*')[1:][1].strip())):
...
при выполнии задания ты можешь указать время
которое затратили наданную задачу и я запомню его
позже ты можешь посмотреть статистику за день, месяц,
неделю или выбранный тобой период
